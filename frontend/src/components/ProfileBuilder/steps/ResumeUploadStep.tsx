    ...
    if (parsedData) {
      ...
      if (parsedData.personal_info) {
        ...
        updateData('basicInfo', {
          ...data.basicInfo,
          firstName: personalInfo.name?.split(' ')[0] || data.basicInfo.firstName || '',
          lastName: personalInfo.name?.split(' ').slice(1).join(' ') || data.basicInfo.lastName || '',
          email: personalInfo.email || data.basicInfo.email || '',
          phone: personalInfo.phone || data.basicInfo.phone || '',
          // Don't overwrite address if it already exists
      ...
      if (parsedData.work_experience) {
        updateData('workExperience', parsedData.work_experience.map(exp => ({
          id: `work-${Date.now()}-${Math.random()}`,
          companyName: exp.company,
          jobTitle: exp.position,
          startDate: exp.start_date,
          endDate: exp.end_date,
          isCurrentRole: false, // Default to false, user can adjust
      ...
      if (parsedData.education) {
        updateData('education', parsedData.education.map(edu => ({
          ...
          startDate: edu.graduation_date, // Using graduation date as start date
      ...
      if (parsedData.skills) {
        ...
        parsedData.skills.forEach(skillGroup => {
          skillGroup.skills.forEach(skill => {
            let skillType: 'technical' | 'soft' | 'language' | 'certification' = 'technical';
            
            if (skillGroup.category.toLowerCase().includes('soft')) {
              skillType = 'soft';
            } else if (skillGroup.category.toLowerCase().includes('language')) {
              skillType = 'language';
            } else if (skillGroup.category.toLowerCase().includes('certification')) {
            ...
            skillsData.push({
              ...
              skillName: skill,
              proficiency: 'intermediate', // Default to intermediate, user can adjust later
              skillType,
      ...
      if (parsedData.projects) {
        updateData('projects', parsedData.projects.map(project => ({

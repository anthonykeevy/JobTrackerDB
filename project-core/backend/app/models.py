...
class Profile(Base):
    ...
    # Personal Information - Reorganized
    DateOfBirth = Column(DateTime)
    CountryOfBirth = Column(Unicode(100))  # New field for country of birth
    CurrentCitizenship = Column(Unicode(100))  # New field for current citizenship
    
    # Contact Information
    PhoneNumber = Column(Unicode(50))
    EmailAddress = Column(Unicode(255), unique=True, nullable=False)
    Website = Column(Unicode(255))  # Keep for backward compatibility
    
    # Legacy fields (deprecated - will be migrated to new structure)
    AddressLine1 = Column(Unicode(255))
    AddressLine2 = Column(Unicode(255))
    AddressLine3 = Column(Unicode(255))
    LinkedInURL = Column(Unicode(255))  # Legacy - will be migrated to ProfileSocialLink
    GitHubURL = Column(Unicode(255))    # Legacy - will be migrated to ProfileSocialLink
    OtherSocialProfiles = Column(Unicode(500))  # Legacy - will be migrated to ProfileSocialLink
    Nationality = Column(Unicode(100))  # Legacy - will be migrated to CurrentCitizenship
    ...
    social_links = relationship("ProfileSocialLink", back_populates="profile", cascade="all, delete-orphan")
...
class ProfileSocialLink(Base):
    """Flexible social/professional links with user-defined names and admin approval"""
    __tablename__ = "ProfileSocialLink"
    
    ProfileSocialLinkID = Column(Integer, primary_key=True, autoincrement=True)
    ProfileID = Column(Integer, ForeignKey("Profile.ProfileID"), nullable=False)
    
    # Link details
    LinkName = Column(Unicode(100), nullable=False)  # User-defined name (e.g., "LinkedIn", "GitHub", "My Blog")
    LinkURL = Column(Unicode(500), nullable=False)
    LinkType = Column(Unicode(50))  # 'professional', 'social', 'portfolio', 'other'
    LinkIcon = Column(Unicode(100))  # Icon class or identifier
    
    # Approval and visibility
    IsApproved = Column(Boolean, default=False)  # Admin approval for new link types
    IsActive = Column(Boolean, default=True)
    IsPublic = Column(Boolean, default=True)  # Whether to show in public profile
    ...
    def __repr__(self):
        return f"<ProfileSocialLink(ProfileID={self.ProfileID}, Name='{self.LinkName}', URL='{self.LinkURL}')>"

class GlobalLinkType(Base):
    """Pre-approved link types that users can select from"""
    __tablename__ = "GlobalLinkType"
    
    GlobalLinkTypeID = Column(Integer, primary_key=True, autoincrement=True)
    LinkName = Column(Unicode(100), nullable=False, unique=True)
    LinkType = Column(Unicode(50), nullable=False)  # 'professional', 'social', 'portfolio'
    LinkIcon = Column(Unicode(100))
    DisplayOrder = Column(Integer, default=999)
    IsActive = Column(Boolean, default=True)
    ...
    def __repr__(self):
        return f"<GlobalLinkType(Name='{self.LinkName}', Type='{self.LinkType}')>"
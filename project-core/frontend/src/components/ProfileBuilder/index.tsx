...
const ProfileBuilder: React.FC = () => {
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      ...
      if (!userSession) {
        console.error('❌ No valid user session - cannot load profile data');
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const profileId = userSession.profileId;
        
        console.log('🔍 Loading profile data for user:', userId, 'profile:', profileId);
        
        const response = await fetch(`http://localhost:8000/api/v1/resume/load-profile/${userId}`);
        if (response.ok) {
    ...
    loadProfileData();
  }, [userSession, authLoading]);

  const loadProfileScore = async () => {
    if (!userSession) {
      console.error('❌ No valid user session - cannot load profile score');
      return;
    }

    try {
      const userId = userSession.userId;
      console.log('🎯 Loading profile score for user', userId);
      const response = await fetch(`http://localhost:8000/api/v1/profile/score/${userId}`);
      if (response.ok) {
        const result = await response.json();
        console.log('📊 Profile score result:', result);
        if (result.success) {
          setProfileScore(result.data);
          console.log('✅ Profile score loaded:', result.data);
        } else {
          console.error('❌ Profile score request failed:', result);
        }
      } else {
        console.error('❌ Profile score HTTP error:', response.status);
      }
    } catch (error) {
  ...
  const saveProfileSection = async (section: string, data: any) => {
    if (!userSession) {
      console.error('❌ No valid user session - cannot save profile data');
  ...
  const nextStep = async () => {
    if (currentStep < STEPS.length - 1) {
      ...
      // Always save the current data, even if it seems empty
      // This ensures the footer Continue button saves the data
          ...
          {isLoading ? (
            ...
            >
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {authLoading ? 'Validating your session...' : 'Loading your profile data...'}
              </h3>
